			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
				{washListItem?.map((product) => (
					<div key={product._id} className="product-item border rounded-lg p-4">
						<img src={product.imageCover} alt={product.title} className="w-full aspect-square object-cover mb-2" />
						...
						<div className="flex flex-col gap-2 mt-4">
							<button className="w-full bg-transparent hover:bg-[#1abc9c] text-[#1abc9c] font-semibold hover:text-white py-2 px-4 border border-[#1abc9c] hover:border-transparent rounded ">
								Add to My cart
							</button>
							<button
