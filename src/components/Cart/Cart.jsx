...
export default function Cart() {
	...
	const [loadingRemoveCartId, setLoadingRemoveCartId] = useState(null);
	...
	async function removeItem(id) {
		setLoadingRemoveCartId(id)
		setLoadingRemoveCart(true);
		let response = await removeSpecificCartItem(id);

		setLoadingRemoveCartId(response.data.data._id)
	...
	return (
		<>
			<div className="relative overflow-x-auto shadow-lg  my-28   ">
				<h1 className="text-center capitalize text-2xl font-bold my-3">total price {cartData.totalCartPrice}</h1>
				<table className="w-fit text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 lg:w-full md:w-full ">
							...
							<tr key={product._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
								...
								<td className="px-6 py-4">
									<Link onClick={() => removeItem(product.product._id)} href="#" className="font-medium text-red-600 dark:text-red-500 hover:underline">
										{loadingRemoveCart &&loadingRemoveCartId == product.product._id  ? <i className="fa-solid fa-spinner fa-spin "></i> : 'Remove'}
									</Link>
								</td>
							</tr>
						))}
